    ...
    <div class="row align-items-center" style="height: 60px">
      ...
      <div class="col-md-6" style="text-align: right;">
		...
		{{-- <button type="button" class="btn btn-primary btn-lg">Nuevo</button> --}}
          <a href="{{ route('patologia.formulario1.create') }}" class="btn btn-primary btn-lg">Nuevo</a>
		{{-- <button type="button" class="btn btn-success btn-lg" target="_blank">Imprimir Lista</button> --}}
		<a href="{{ route('patologia.formulario1.pdf') }}" class="btn btn-success btn-lg" target="_blank">Imprimir Lista</a> 
        </div>
      </div>
    </div>
    <!-- end row -->
				...
				<div class="table-wrapper table-responsive">
		            <table class="table table-striped" id="myTable">
		              ...
		              <tbody>
		                ...
		                <tr>		                  
		                  <td class="min-width">
		                    <p>{{$formulario1->num_solicitud}}</p>
		                  </td>		            
						  <td class="min-width">
		                    <p>{{$formulario1->fecha_solicitud}}</p>
		                  </td>		            
						  <!--<td class="min-width">
		                    <p>{{$formulario1->paciente}}</p>
		                  </td>
		                  <td class="min-width">
						  ...
						  <td width="15px">
						  	<form action="{{ route('patologia.formulario1.destroy', $formulario1->id) }}" method="POST" onsubmit="return confirm('¿Estás seguro de que deseas eliminar este registro?');">
								@method('delete')
								@csrf
